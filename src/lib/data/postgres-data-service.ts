...
export class PostgresDataService implements IPatientDataService {
  async getPatient(patientId: string) {
    const p = await prismaClient.patient.findUnique({ where: { id: patientId } });
    return p ? toPatient(p) : null;
  }

  async listPatients() {
    const rows = await prismaClient.patient.findMany({ orderBy: { id: "asc" } });
...
export const patientDataService: IPatientDataService = new PostgresDataService();