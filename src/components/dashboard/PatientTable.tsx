...
export default function PatientTable() {
  ...
  const [runningPatientId, setRunningPatientId] = useState<number | null>(null);
  ...
  // Process a patient (fire agent + mark as proceeded)
  async function handleProcess(patientId: number) {
    setRunningPatientId(patientId);
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        throw new Error(data.message || "Failed to process patient");
      }

      // Optimistically update local state so the buttons flip immediately
      setPatients((prev) =>
        prev.map((p) => (p.id === patientId ? { ...p, isProceed: true } : p)),
      );
    } catch (err) {
      console.error(err);
        ...
        <table className="w-full text-left text-sm text-slate-600 dark:text-slate-300">
          ...
          <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
            {patients.map((patient) => {
              const canView = patient.isProceed === true;
              const canProcess = !patient.isProceed;
              const isRunning = runningPatientId === patient.id;
                ...
                >
                  ...
                  <td className="whitespace-nowrap px-6 py-4 text-right">
                    <div className="flex justify-end gap-2">
                      <button
                        onClick={() => {
                          router.push(`/apa-agent/${patient.id}`);
                        }}
                        disabled={!canView}
                      ...
                      <button
                        onClick={() => {
                          handleProcess(patient.id);
                        }}
                        disabled={!canProcess || isRunning}
